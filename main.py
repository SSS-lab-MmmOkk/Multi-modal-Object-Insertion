...
from core.sensor_simulation.lidar_simulator import mix_pc, complet_pc, lidar_simulation
from utils import calibration_kitti
from Utils_label import read_labels_2, write_labels_2, update_bg_labels_care, del_labels, get_care_labels, sort_labels
from Utils_o3d import pc_numpy_2_o3d, load_normalized_mesh_obj
from Utils_common import read_Bin_PC, extact_initial_objs_from_bg, get_initial_box3d_in_bg, get_geometric_info, \
    update_occ_only_image, change_3dbox, get_truncation_ratio, get_labels
from Utils_shapenet import filter_cars
from copy_data import copy_data4epnet, copy_data4fconv, copy_data4clocs, copy_data4second, copy_data4rcnn
from core.occusion_handing.combine_img import combine_bg_with_obj
from core.occusion_handing.combine_pc import combine_pcd, get_occlusion_level, update_init_label
from core.pose_estimulation.collision_detection import collision_detection, is_valid_pc_box, is_on_road
from core.pose_estimulation.pose_generator import generate_pose, tranform_mesh_by_pose, get_valid_pints
from core.pose_estimulation.road_split import road_split
from core.sensor_simulation.camera_simulator import camera_simulation
from evaluate_script import run_epnet, run_fconv, run_clocs, run_second, run_rcnn
from fitness_score import cal_fitness_score, get_objs_attr, get_iou2d
from logger import CLogger
from utils.box_utils import get_2d_box_from_image, get_2d_box_from_points, get_2d_box_center, trunc_2d_box, \
    boxes3d_kitti_camera_to_imageboxes, iou_2d
...
if __name__ == "__main__":
    ...
    parser.add_argument('--select_size', type=int, default=250)
    ...
    for bg_index in tqdm(bg_index_list):
        ...
        os.makedirs(save_image_dir_fitness, exist_ok=True)
        ...
        if str(system_name).lower() == "random" or "random" in str(system_name):
            score_pre = 0
        else:
            dt_path_ori = f"{workplace_dir}/{system_name}/result_ori/{bg_index:06d}.txt"
            score_pre = cal_fitness_score(dt_path_ori, bg_label_path, system_name, img=img_bg,
                                          save_path=save_image_dir_fitness,
                                          ix=0, modality=modality)
        _objs_index_arr = []
        _pcds = []
        _meshes = []
        _objs_img = []
        ...
        while i < objs_inserted_max_num:
            ...
            CLogger.info(f"insert {i + 1}: {obj_name} {obj_car_dirs[objs_index]},fitness: {score_pre}")
            ...
            while try_num > 0:
                ...
                CLogger.debug(f"pos:{position},rz:{rz_degree}")
            ...
            if try_num <= 0:
                CLogger.info(f"over {i + 1} /{config.algorithm_config.try_num}")
                if i == 0:
                    CLogger.info("no obj insert successfully, skip")
                    i = np.inf
            ...
            if str(system_name).lower() == "random" or "random" in str(system_name):
                score = 0
                score_flag = True
            elif "EPNet" in str(system_name):

                copy_data4epnet(f"{bg_index:06d}")

                run_epnet(guided=True)

                dt_path = f"{workplace_dir}/{system_name}/result/{bg_index:06d}.txt"
                score = cal_fitness_score(dt_path, save_label_path, system_name, img=img_mix_gbr,
                                          save_path=save_image_dir_fitness,
                                          ix=i + 1, modality=modality)
                score_flag = score > score_pre
            elif "FConv" in str(system_name):

                copy_data4fconv(f"{bg_index:06d}")

                run_fconv(guided=True)
                ...
                score = cal_fitness_score(dt_path, save_label_path, system_name, img=img_mix_gbr,
                                          ...
                                          ix=i + 1, modality=modality)
                score_flag = score > score_pre
            elif "CLOCs" in str(system_name):
                copy_data4clocs(f"{bg_index:06d}")

                run_clocs(guided=True)
                ...
                score = cal_fitness_score(dt_path, save_label_path, system_name, img=img_mix_gbr,
                                          ...
                                          ix=i + 1, modality=modality)
                score_flag = score > score_pre
            elif "Second" in str(system_name):
                copy_data4second(f"{bg_index:06d}")

                run_second(guided=True)
                ...
                score = cal_fitness_score(dt_path, save_label_path, system_name, img=img_mix_gbr,
                                          ...
                                          ix=i + 1, modality=modality)
                score_flag = score > score_pre
            elif "Rcnn" in str(system_name):
                copy_data4rcnn(f"{bg_index:06d}")

                run_rcnn(guided=True)
            ...
            if score_flag or j == 5:
                if j == 5:
                    CLogger.info(f"data {bg_index} insert {i} objs\n, finess {score}, score no change")
                    with open(debug_log, 'a') as f:
                        f.writelines(f"data {bg_index} insert {i} objs\n, finess {score}, score no change")
                else:
                    score_pre = score
                i += 1
                j = 0
                CLogger.info("fitness_score: {} ->{} ,next iter, time {}".format(score_pre, score, per_time))
                ...
                j += 1
                CLogger.info("fitness_score no change, time {}".format(per_time))

                continue

        if i == np.inf:
            CLogger.warning(f"skip data {bg_index}, no object insert")
            with open(debug_log, 'a') as f:
                f.writelines(f"data {bg_index} insert 0 objs\n, no object insert")
            continue
        else:
            ...
            if system_name == "random" or "random" in str(system_name):
                ...
            else:
                shutil.copyfile(dt_path, final_result_path)
